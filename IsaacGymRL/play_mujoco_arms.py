...
if __name__ == "__main__":
    ...
    oldActions = np.zeros(default_dof_pos.shape, dtype=np.float32)
    dof_pos = mj_data.qpos.astype(np.float32)[7:]
    pref_dof_pos = np.zeros(dof_pos.shape, dtype=np.float32)
    pref_dof_pos[:] = mj_data.qvel.astype(np.float32)[6:]
    ...
    arms_shift = 8 if cfg["algorithm"]["use_arms"] else 0
    ...
    with mujoco.viewer.launch_passive(mj_model, mj_data) as viewer:
        ...
        while viewer.is_running():
            ...
            last_id = len(dof_targets) - 1
            ankle_ids = [last_id, last_id-1, last_id-6, last_id-7]
            all_ids = list(range(len(dof_targets)))
            other_joint_ids = [i for i in all_ids if i not in ankle_ids]
            current_vel = (dof_pos[:] - pref_dof_pos[:]) * 500
            ...
            if it % cfg["control"]["decimation"] == 0:
                ...
                obs[9] = 1
                obs[10] = 1
                obs[11] = np.cos(2 * np.pi * gait_process) * (gait_frequency > 1.0e-8)
                obs[12] = np.sin(2 * np.pi * gait_process) * (gait_frequency > 1.0e-8)
                obs[13:25+waist_shift+arms_shift] = (dof_pos[num_of_unknown_joints:] - default_dof_pos[num_of_unknown_joints:]) * cfg["normalization"]["dof_pos"]
                obs[25+waist_shift+arms_shift:37+2*(waist_shift+arms_shift)] = current_vel[num_of_unknown_joints:] * cfg["normalization"]["dof_vel"]
                obs[37+2*(waist_shift+arms_shift):50+3*(waist_shift+arms_shift)] = actions
                idx = 50+3*(waist_shift+arms_shift)
                obs[idx:idx+8] = 0
                obs[idx+8] = 0
                obs[idx+9] = 1
                obs[idx+10] = 0
