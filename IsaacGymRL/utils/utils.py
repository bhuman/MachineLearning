...
def apply_randomization(tensor, params, return_noise=False):
    ...
    elif params["distribution"] == "uniform_log":
        lower, upper = params["range"]
        noise = torch.rand_like(tensor) if isinstance(tensor, torch.Tensor) else np.random.rand()
        log_scale = (
            torch.log(torch.rand_like(tensor).clamp(min=0.0067, max=0.37))
            if isinstance(tensor, torch.Tensor)
            else np.log(np.clip(np.random.rand(*np.shape(tensor)), a_min=0.0067, a_max=0.37))
        )
        noise_val = lower + (upper - lower) * noise / -log_scale
    else:
...
def surrogate_loss(old_actions_log_prob, actions_log_prob, advantages, e_clip):
